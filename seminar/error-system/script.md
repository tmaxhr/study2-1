...
마지막에 Q&A와 같은 페이지가 따로 없으니, 발표 중간중간에 물어보면 좋겠다
...
잘못된 데이터를 접근하는 경우가 있다. 물론 현재 Typescript를 쓰면서 어느정도 런타임 시의 에러를 방지하고 있지만, (any를 사용한 곳이 많기 때문에) 종종 발생한다.

# 화면 8

우리가 이런 고민들을 하는 목표는 런타임에서 발생하는, 즉 예상하지 못한 에러들을 적절히 대처하고, 사용자에게 보다 더 나은 경험을 제공해주기 위한 것
...
이렇게 로깅을 할 때 담당자를 어떻게 빨리 찾아낼 수 있을까 고민하다가, 떠올린 것이 도메인 별로 에러 코드를 분리하는 것이다.
