...
2실 3명, 가벼운 주제로 준비했음. 이론적인 이야기보다는 어떤 고민들을 했고 어떻게 사용했는지를 위주로 되어있으니, 발표 중간중간에 물어보면 좋겠다.

마지막에 Q&A와 같은 페이지가 따로 없다.
...
또한 모든 에러를 catch 해주는 것은 휴먼 에러의 가능성을 배제할 수 없다.
...
제일 먼저 생각나는건 앞선 예시와 같이 api 호출이 실패했을 때.

원인은 네트워크 오류, fetch 자체가 실패했을 때, 서버에서 응답은 왔지만 성공하지 않았을 경우 등 다양한 경우가 있음

# 화면 7

잘못된 데이터를 접근하는 경우가 있다. 물론 현재 Typescript를 쓰면서 어느정도 런타임 시의 에러를 방지하고 있지만, 기간계 프로젝트에서는 any를 사용한 곳이 많기 때문에 발생할 가능성이 높다.

# 화면 8

개발자가 던진 에러 (앞서 말한 것과 동일)

# 화면 9

우리가 이런 고민들을 하는 목표는 런타임에서 발생하는, 즉 사전에 처리하지 못한 에러들을 적절히 대처하고, 사용자에게 보다 더 나은 경험을 제공해주기 위한 것

# 화면 10

그럼 어떻게 처리했을까. 우리는

1. 이런 에러 처리에 있어 권장되는 방식이 있는지,
...
context api를 사용할 수도 있고 다른 방법들이 많지만 에러 바운더리를 선택한 이유는
보일러플레이트가 적고, try-catch를 일일이 작성하지 않아도 되는 등의 장점 때문이다.

에러 바운더리는 리액트 16에서 처음 등장한 개념으로, 트리 구조로 이루어진 JSX에서 에러가 발생한 하위 컴포넌트 트리를 탐지하고 fallback UI를 보여주는 컴포넌트이다. 자바스크립트의 catch와 비슷하다고 생각하면 됨

# 화면 12

그래서 Error boundary를 통해 이런 에러들을 적절히 분류해 처리하고자 했다.

# 화면 13

이 Error boundary를 우리는 RootBoundary, PageBoundary 두 개의 바운더리로 나누었다.
둘다 에러 바운더리를 감싼 컴포넌트이다.
...
root boundary 코드 설명, page500으로 라우팅 하는 코드만 있음

그럼 Error boundary에 문제점은 없을까?

# 화면 16

Error boundary에서는 비동기 작업(setTimeout 같은)과 이벤트 핸들러에서 발생한 에러는 캐치하지 못한다.
...
슬랙에 로깅하는 코드를 아까 말했던 RootBoudary에 넣으면, slack을 통해 어떤 부분에서 에러가 발생했는지 알 수 있다.

# 화면 21

또한 아까 말했던, 페이지 단위로 에러를 처리하는 PageBoundary에서도 로깅을 하고 싶다면 log 값을 true로 설정해 로깅을 할 수 있다.

이렇게 로깅을 할 때 담당파트를 어떻게 빨리 파악할 수 있을까 고민하다가, 떠올린 것이 도메인 별로 에러 코드를 분리하는 것이다.
...
이렇게 하면 단순히 에러만 뿜는 것보다 어떤 파트인지, 담당자가 누구인지 보다 다양한 정보를 보여줄 수 있고

이에 따라 커뮤니케이션 비용도 줄고, 에러 대응 속도도 빨라질 것이라 기대한다.

# 화면 25 (FIN)

에러 처리는 서비스가 실 사용자에게 서빙될 때 빠져서는 안되는 필수 기능이라고 생각함. 에러들을 어떻게 해결할지를 고민하는건 UX를 고민하는 것과 같다.

프론트는 사용자와 직접 맞닿는 영역이기 때문에, 적절한 에러 처리는 UX의 향상 뿐만 아니라 서비스에 대한 완성도와 신뢰도 또한 높여줄 수 있음.

굳이 우리의 방법이 아니더라도, 사내 다양한 프로젝트에서 에러에 대한 고민을 한 번쯤은 고려해보게 된다면 성공적인 세미나가 되었다고 생각한다.
